    ...
    function eZOracleDB( $parameters )
    {
        ...
        if ( function_exists( "OCILogon" ) )
        {
            ...
            if ( $ini->variable( "DatabaseSettings", "UsePersistentConnection" ) == "enabled" )
            {
                ...
                $this->DBConnection = OCIPLogon( $user, $password, $db, $oraCharset );
    ...
    function analizeQuery( $sql, $server = false )
    {
        ...
        if ( $this->QueryAnalysisOutput )
        {
            ...
            if ( $analysisResult )
            {
                ...
                OCIFreeStatement( $analysisStmt );
                $analysisStmt = OCIParse( $this->DBConnection, "SELECT LPAD(' ',2*(LEVEL-1))||operation operation, options,
...
//        $currentCharset = eZCharsetInfo::realCharsetCode( $currentCharset );
    ...
    */
    ...
    function databaseServerVersion()
    {
        ...
        $versionInfo = ociserverversion( $this->DBConnection );
