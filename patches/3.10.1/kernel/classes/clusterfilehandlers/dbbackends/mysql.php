...
class eZDBFileHandlerMysqlBackend
{
    ...
    function _copyInner( $srcFilePath, $dstFilePath, $fname, $metaData )
    {
        ...
        while ( $row = mysql_fetch_row( $res ) )
        {
            ...
            {
                $this->_error( "The fetched offset value '$expectedOffset' does not match the computed one for the file '$srcFilePath', aborting copy.", $fname );
                return false;
            }
        ...
        {
            $this->_error( "The size of the fetched data '$offset' does not match the expected size '$contentLength' for the file '$srcFilePath', aborting copy.", $fname );
            return false;
        }
...
//        $tmpFilePath = $filePath.getmypid().'tmp';
    ...
    function _fetchContents( $filePath, $fname = false )
    {
        ...
        if ( !$metaData )
        {
            $this->_error( "File '$filePath' does not exists while trying to fetch its contents.", $fname );
            return false;
        }
        $contentLength = $metaData['size'];
...
//        $fileID = $metaData['id'];
        ...
        if ( !$res = $this->_query( $sql, $fname ) )
        {
            $this->_error( "Failed to fetch file data for file '$filePath'.", $fname );
            return false;
        }
        ...
        while ( $row = mysql_fetch_row( $res ) )
        {
            ...
            if ( $expectedOffset != $offset )
            {
                $this->_error( "The fetched offset value '$expectedOffset' does not match the computed one for the file '$filePath', aborting fetchContents.", $fname );
                return false;
            }
            $contents .= $row[0];
            $offset += strlen( $row[0] );
        ...
        if ( $offset != $contentLength )
        {
            $this->_error( "The size of the fetched data '$offset' does not match the expected size '$contentLength' for the file '$filePath', aborting fetchContent.", $fname );
            return false;
        }
...
//////////////////////////////////////
    ...
    /*!
     A kind of two-phse-commit lock verification process:
     use a separate db connection to check the mtime set to the file.
     Ugly API needed for backward compatibility with code that was in ezDBFileHandler

     \return true if file is locked correctly (ie mtime negative), false otherwise
     */
    function _verifyExclusiveLock( $filePath, $expiry, $curtime, $ttl, $fname = false )
    {
        static $backendVerify;
        if ( $backendVerify === null )
        {
            $backendclass = get_class( $this );
            $backendclass =& new $backendclass();
            $backendVerify =& $backendclass;
            $backendclass->_connect( true );
        }
        $metaData = $backendVerify->_fetchMetadata( $fname );
        if ( $metaData !== false )
        {
        ...
        // note: this should probably return false: since there is no file metadata,
        // it means that something wrong has gone on...
        return true;
    }
     ...
     */
    function _error( $query, $fname, $error = false )
    {
        if ( $error === false )
