        ...
        while ( true )
        {
            ...
            while ( true )
            {
                ...
                else
                {
                    ...
                    if ( $mtime === false || eZDBFileHandler::isExpired( $fname, $mtime, $expiry, $curtime, $ttl ) )
                    {
                        // Local file is older than global timestamp, check with DB
                        eZDebugSetting::writeDebug( 'kernel-clustering', "Local file (mtime=$mtime) is older than timestamp ($expiry) and ttl($ttl), checking with DB" );
                        $forceDB = true;
                    }
                }
                ...
                if ( !$forceDB )
                {
                    ...
                    if ( $this->metaData === false || $this->metaData['mtime'] < 0 )
                    {
                        if ( $generateCallback !== false )
                            eZDebugSetting::writeDebug( 'kernel-clustering', "Local file OK but database file is deleted, need to regenerate data" );
                        else
                            eZDebugSetting::writeDebug( 'kernel-clustering', "Local file OK but database file is deleted, cannot get data" );
                        break;
                    }
...
//                      if ( $this->metaData === false || eZDBFileHandler::isExpired( $fname, $mtime, $expiry, $curtime, $ttl ) )
                    ...
                    {
                        eZDebugSetting::writeDebug( 'kernel-clustering', "Processing local file $fname, $mtime, $ttl, $expiry" );
                ...
                if ( !eZDBFileHandler::isExpired( $fname, $this->metaData['mtime'], $expiry, $curtime, $ttl ) )
                {
                    ...
                    else
                    {
                        ...
                        $args = array( $path, $this->metaData['mtime'] );
                        if ( $extraData !== null )
                            $args[] = $extraData;
                        $retval = call_user_func_array( $retrieveCallback, $args );
                        $this->fileDeleteLocal( $path );
                        if ( $retval instanceof eZClusterFileFailure )
                        {
                            break;
                        }
                ...
                eZDebugSetting::writeDebug( 'kernel-clustering', "Database file (mtime=".$this->metaData['mtime'].") is older than timestamp ($expiry) and ttl($ttl)" );
                break;
            }
            ...
            if ( $generateCallback !== false )
            {
                ...
                $res = $this->backend->_verifyExclusiveLock( $fname, $expiry, $curtime, $ttl, 'processCache' );
                if ( !$res )
                {
    ...
    function fileDelete( $path, $fnamePart = false )
    {
        ...
        eZDebugSetting::writeDebug( 'kernel-clustering', "db::fileDelete( '$path', '$fnamePart' )" );

        if ( $fnamePart === false )
        {
            $this->backend->_delete( $path );
            unset( $GLOBALS['eZClusterInfo'][$path] );
            $pattern = $path . '/%';
        }
        else
    ...
    function purge( $printCallback = false, $microsleep = false, $max = false, $expiry = false )
    {
        ...
        $this->backend->_purge( $file, true, $expiry, 'purge' );
        $this->metaData = false;
        unset( $GLOBALS['eZClusterInfo'][$file] );
        // Remove local copy
        if ( is_file( $file ) )
        {
            @unlink( $file );
    ...
    function fileMove( $srcPath, $dstPath )
    {
        ...
        //$this->metaData = false;
        unset( $GLOBALS['eZClusterInfo'][$srcPath] );
    }
