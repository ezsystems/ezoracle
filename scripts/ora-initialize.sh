...
if [ -z "$EZNETADMIN" ] || [ ! -f "$EZNETADMIN/tnsnames.ora" ]; then
    ...
    echo
    echo "  Please note that eZ Publish does not currently support Oracle Easy"
    echo "  Naming connections"
    echo
...
{
    \$statement = ociParse( \$db, "select VALUE from NLS_DATABASE_PARAMETERS where PARAMETER='NLS_CHARACTERSET'" );
    if ( !ociExecute( \$statement, OCI_DEFAULT ) )
    {
        \$error = ociError( \$statement );
        if ( \$error['code'] != 0 )
